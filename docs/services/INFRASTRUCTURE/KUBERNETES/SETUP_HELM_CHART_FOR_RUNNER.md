...
Create a production-ready Helm Chart for the Runner service. This standardizes the deployment process, allowing developers to deploy the application to any Kubernetes cluster (Dev/Staging/Prod) using a single command with configurable parameters.
...
- Current state: Developers are not able to deploy Runner to K8s cluster.
...
```bash
helm upgrade --install runner ./charts/runner -f values.prod.yaml
```
...
replicaCount: 2
image:
  repository: devyusupov/statushawk-runner
  tag: "latest"
service:
  type: ClusterIP
  port: 8000
...
- **Linting**: Run `helm lint ./charts/runner` in CI to catch syntax errors.
- **Dry Run**: Run `helm template ./charts/runner` to verify the generated YAML looks correct without deploying it.
