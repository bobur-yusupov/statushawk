...
def test_user() -> "AbstractBaseUser":
    return User.objects.create_user(
        email="test@example.com", password="SecurePassword!123"
    )
...
@pytest.mark.django_db
def test_signup_unsupported_media_type(test_user: AbstractBaseUser) -> None:
    client = APIClient()
    payload: str = (
        "first_name=John&last_name=Doe&email=unsupported@example.com"
        "&password=SecurePassword!123"
    )
...
@pytest.mark.django_db
def test_signup_rate_limiting(api_client: APIClient) -> None:
    payload: Dict[str, str] = {
        "first_name": "John",
        "last_name": "Doe",
