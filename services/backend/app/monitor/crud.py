...
class MonitorResultCRUD(FullCRUD[MonitorResult]):
    ...
    def filter_by_monitor_and_period(self, monitor: Monitor, start_time: timezone.datetime) -> QuerySet[MonitorResult]:
        return self.model.objects.filter(monitor=monitor, created_at__gt=start_time)
    ...
    def get_stats_aggregate(self, queryset: QuerySet[MonitorResult]) -> Dict[str, Any]:
        return queryset.aggregate(
            ...
            down_count=Count(Case(When(is_up=False, then=1), output_field=IntegerField())),
            avg_latency=Avg("response_time_ms"),
        )
    
    def get_last_check(self, monitor: Monitor) -> Optional[MonitorResult]:
        return self.model.objects.filter(monitor=monitor).order_by("-created_at").first()
    
    def get_history(self, monitor: Monitor, period: Optional[str] = None) -> QuerySet[MonitorResult]:
        queryset = self.model.objects.filter(monitor=monitor).order_by("-created_at")
...
class MonitorCRUD(FullCRUD[Monitor]):
    ...
    def filter_by_user(self, user: Any, is_active: Optional[bool] = None) -> QuerySet[Monitor]:
