...
class TestMonitorCRUD:
    ...
    def test_create(self, user: Any) -> None:
        crud = MonitorCRUD()
        monitor = crud.create(user=user, name="Test", url="https://test.com", monitor_type="HTTP")
...
class TestMonitorResultCRUD:
    ...
    def test_get_history(self, monitor: Monitor) -> None:
        crud = MonitorResultCRUD()
        MonitorResult.objects.create(monitor=monitor, status_code=200, response_time_ms=100, is_up=True)
        MonitorResult.objects.create(monitor=monitor, status_code=200, response_time_ms=150, is_up=True)
