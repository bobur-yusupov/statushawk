...
class TestMonitorService:
    ...
    def test_create(self, service: MonitorService, user: Any) -> None:
        monitor = service.create(user=user, name="Test", url="https://test.com", monitor_type="HTTP")
    ...
    def test_get_stats_with_failures(self, service: MonitorService, monitor: Monitor) -> None:
        MonitorResult.objects.create(monitor=monitor, status_code=200, response_time_ms=100, is_up=True)
        MonitorResult.objects.create(monitor=monitor, status_code=500, response_time_ms=200, is_up=False)
    ...
    def test_get_stats_no_results(self, service: MonitorService, monitor: Monitor) -> None:
    ...
    def test_get_history_with_period(self, service: MonitorService, monitor: Monitor) -> None:
        now = timezone.now()
    ...
    def test_get_dashboard_stats_recent_failures(self, service: MonitorService, user: Any, monitor: Monitor) -> None:
        for i in range(3):
            MonitorResult.objects.create(monitor=monitor, status_code=500, response_time_ms=100, is_up=False)
        
        stats = service.get_dashboard_stats(user)
        assert len(stats["recent_failures"]) == 3
        assert stats["recent_failures"][0]["monitor_name"] == monitor.name

    def test_get_dashboard_stats_inactive_monitors(self, service: MonitorService, user: Any) -> None:
        Monitor.objects.create(user=user, name="M1", url=fake.url(), monitor_type="HTTP", is_active=True)
        Monitor.objects.create(user=user, name="M2", url=fake.url(), monitor_type="HTTP", is_active=False)
